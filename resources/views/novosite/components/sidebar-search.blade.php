...
<section {{ $attributes->merge(['class' => 'w-full group/search']) }} role="search" aria-labelledby="search-title">
    ...
    <form action="{{ route('site.post.list') }}" method="GET" class="relative">
        ...
        <div
            class="relative bg-white border-[3px] border-[#002266] rounded-full shadow-[4px_4px_0px_0px_#002266] transition-all duration-300 group-focus-within/search:shadow-[6px_6px_0px_0px_#A4ED4A] group-focus-within/search:translate-x-[-2px] group-focus-within/search:translate-y-[-2px] overflow-hidden">
        ...
        <div class="mt-4 flex justify-between items-center px-4" aria-hidden="true">
            <div class="flex gap-1.5 items-center">
                <div
                    class="w-3 h-3 bg-[#A4ED4A] border-2 border-[#002266] rotate-45 group-focus-within/search:bg-[#002266]">
                </div>
                <div class="w-12 h-[2px] bg-[#002266]/10"></div>
            </div>
            <span
                class="text-[9px] font-[900] text-[#002266] uppercase tracking-[0.3em] opacity-40 group-focus-within/search:opacity-100 transition-opacity">
                Aguardando_Input
            </span>
        </div>
    </form>
