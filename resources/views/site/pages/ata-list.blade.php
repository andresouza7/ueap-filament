...
@section('content')
        {{--  <form class='form' action="{{ route('site.consu.ata.list') }}" method="get">
            <div class='row m-2'>
                    ...
                    <div class='col-2'>
                        <a style='color:white !important;' class="form-control btn btn-secondary" href='{{ route('site.consu.resolution.list') }}'>Limpar Filtro</a>
                    </div>
            ...
            <ul>
                @forelse($atas as $ata)
                    @if(file_exists(public_path("storage/documents/atas/".$ata->id.".pdf")))
                        <li><a target='blank' title="{{$ata->title}}" href="{{ asset("storage/documents/atas/".$ata->id.".pdf") }}">
