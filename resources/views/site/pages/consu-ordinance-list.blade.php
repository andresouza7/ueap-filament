...
@section('content')
        <form class='form' action="{{ route('site.document.consu-ordinance.list') }}" method="get">
            <div class='row m-2'>

                    <div class='col-4'>
                        <input autocomplete="off"  class="form-control busca" placeholder="Assunto / Descrição" type="text" name="name" value="{{ request('name') }}">
                    </div>
                    ...
                    <div class='col-2'>
                        <a style='color:white !important;' class="form-control btn btn-secondary" href='{{ route('site.document.consu-ordinance.list') }}'>Limpar Filtro</a>
                    </div>
            ...
            <ul>
                @forelse($ordinances as $ordinance)
                    @if(file_exists(public_path("storage/documents/ordinances/".$ordinance->id.".pdf")))
                        <li><a target='blank' title="{{$ordinance->title}}" href="{{ asset("storage/documents/ordinances/".$ordinance->id.".pdf") }}">
                            <span>{{$ordinance->number}}/{{$ordinance->year}} - <b>{{ $ordinance->subject}}</b> - </span>{{$ordinance->description}}</a>
                        </li>
                    @else
