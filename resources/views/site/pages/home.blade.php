...
@section('content')
    ...
    <div id="carouselExampleDark" class="carousel carousel-dark slide" data-bs-ride="carousel">
        @php
            $banners = \App\Models\WebBanner::where('web_banner_place_id', 1)
    ...
    <div class='row m-1'>
        ...
        <div class='col-4 col-md-2 icon border p-1'>
            <a href='{{ route('old.site.document.list', 'calendar') }}'>
        ...
        <div class='col-4 col-md-2 icon border  p-1'><a href='pagina/legislacao.html'>
                <img class='img-fluid' src='{{ asset('site_antigo/img/icone_legislacao.png') }}' /><br />Legislação
                <br />Ueap</a></div>
        <div class='col-4 col-md-2 icon border  p-1'><a href='pagina/instrucoes_normativas.html'>
                <img class='img-fluid' src='{{ asset('site_antigo/img/icone_instrucao_normativa.png') }}' /><br />Instruções
                Normativas</a></div>
        <div class='col-4 col-md-2 icon border  p-1'><a href='{{ route('old.site.document.resolution.list') }}'>
                <img class='img-fluid' src='{{ asset('site_antigo/img/icone_resolucao.png') }}' /><br />Resoluções
                CONSU</a></div>
        ...
        <div class='col-4 col-md-2 icon border'><a href='{{ route('transparency.bid.list') }}'>
                <img class='img-fluid  p-1'
                    src='{{ asset('site_antigo/img/icone_licitacoes.png') }}' /><br />Licitações</a></div>
        <div class='col-4 col-md-2 icon border  p-1'><a href='pagina/area-processos-seletivos.html'>
                <img class='img-fluid' src='{{ asset('site_antigo/img/icone_processo_seletivo.png') }}' /><br />Processos
                Seletivos</a></div>
    ...
    <ul>
        <li><a href='{{route('old.site.page.show', "resultado-editais-2024-provisorio-.html")}}'>Editais PROEXT</a></li>
        <li><a href='{{route('old.site.page.show', "editais-e-arquivos-provis-rios-propesp-2024.html")}}'>Editais PROPESP </a></li>
        <li><a href='{{route('old.site.page.show', "editais-prograd-2024.html")}}'>Editais PROGRAD </a></li>
        <li><a href='{{route('old.site.page.show', "processos-seletivos-ueap.html")}}'>Editais DIPS </a></li>
    </ul>
</div> --}}
    {{-- provisório --}}

    <div id="lista_noticias">
        <div class='titulo'>Últimas Notícias (<a href='{{ route('old.site.post.list') }}'>Ver Todas</a>)</div>
        <ul>


            @foreach ($posts as $post)
                <li> <span>{{ $post->created_at->format('d/m/Y') }}</span> -
                    <a title=" {{ $post->title }}" href='{{ route('old.site.post.show', $post->slug) }}'>
                        {{ $post->title }}
                    </a>
                </li>
            @endforeach
        ...
        <ul>
            @foreach ($events as $event)
                <li> <span>{{ $event->created_at->format('d/m/Y') }}</span> -
                    <a title=" {{ $event->title }}" href='{{ route('old.site.post.show', $event->slug) }}'>
                        {{ $event->title }}
                    </a>
                </li>
            @endforeach
